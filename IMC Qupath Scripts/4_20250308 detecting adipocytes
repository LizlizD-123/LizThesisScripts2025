...
def pathModel = 'D:/ElizabethD/20241217 Qupath cellpose training IMC/models/Custom_model_2025-03-08_14_12.cpm'
def cellpose = Cellpose2D.builder( pathModel )
        .pixelSize( 1)                 // Resolution for detection in um
        .channels( 'Channel 2' )	               // Select detection channel(s)
...
//        .preprocess( ImageOps.Filters.median( 1 ) )                // List of preprocessing ImageOps to run on the images before exporting them
 //       .normalizePercentilesGlobal( 0.1, 99.8, 1 ) // Convenience global percentile normalization. arguments are percentileMin, percentileMax, dowsample.
...
//        .cellposeChannels( 1,2 )           // Overwrites the logic of this plugin with these two values. These will be sent directly to --chan and --chan2
        .cellprobThreshold( 0.7 )          // Threshold for the mask detection, defaults to 0.0
        .flowThreshold( 0.38)              // Threshold for the flows, defaults to 0.4
