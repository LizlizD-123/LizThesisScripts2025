...
```{r}
#happy with how it looks in Mantis and the UMAP
...
speAPC_clustered <- speAPC_clustered |>
  mutate(Cluster_APC_final = case_when(
    APC_Cluster == "cluster_1" ~ "pDC", # Granulocytes
    APC_Cluster == "cluster_2" ~ "Langerin+ cDC2", # Myeloid cells
    APC_Cluster == "cluster_3" ~ "cDC2", # Squamous cells
    APC_Cluster == "cluster_4" ~ "CD169- macs", # Epithelial cells
    APC_Cluster == "cluster_5" ~ "cDC1", # Squamous cells (tumour cells)
    APC_Cluster == "cluster_6" ~ "CD169+ macs", # CD4 T cells
    APC_Cluster == "cluster_7" ~ "Mono/mac", # B cells
...
speT_clustered<- speT_clustered |>
  mutate(Cluster_T_final = case_when(
    T_Cluster == "cluster_1" & Cluster_T_memory== "memory" ~ "Memory CD4+ T cell", # Granulocytes
    T_Cluster == "cluster_2" & Cluster_T_memory== "memory" ~ "Memory CD8+ T cell",
    T_Cluster == "cluster_1" & Cluster_T_memory== "naive" ~ "Naive CD4+ T cell", # Granulocytes
...
speCD45_clustered <- speCD45_clustered |>
  mutate(Cluster_CD45_final = case_when(
    ...
    Cluster_CD45_before_final == "LEC" ~ "LEC", # Myeloid cells
